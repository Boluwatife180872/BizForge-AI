...
import { Plus, ExternalLink, Sparkles, LayoutGrid } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-10">
          ...
          <Link
            href="/create"
            className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-[var(--accent-foreground)] text-sm font-medium transition-all hover:-translate-y-0.5"
          >
        ...
        {businesses.length > 0 && (
          <div className="grid grid-cols-3 gap-4 mb-10">
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5">
              ...
              <p className="text-2xl font-semibold tracking-tight">{businesses.length}</p>
            </div>
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5">
              <p className="text-xs text-[var(--muted-foreground)] mb-1">Products</p>
              <p className="text-2xl font-semibold tracking-tight">{totalProducts}</p>
            </div>
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5">
              <p className="text-xs text-[var(--muted-foreground)] mb-1">Marketing assets</p>
              <p className="text-2xl font-semibold tracking-tight">{totalAssets}</p>
            </div>
          </div>
        )}
        ...
        {businesses.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 border border-dashed border-[var(--border)] rounded-xl">
            ...
            <Link
              href="/create"
              className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-[var(--accent-foreground)] text-sm font-medium transition-all hover:-translate-y-0.5"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {businesses.map((biz) => (
              <Link
                ...
                className="group block rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-5 hover:border-[var(--border)] hover:bg-[var(--background)] transition-all hover:shadow-sm"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <h3 className="text-sm font-medium text-[var(--foreground)] truncate">{biz.name}</h3>
                ...
                <p className="text-xs text-[var(--muted)] line-clamp-2 mb-4 leading-relaxed">{biz.tagline}</p>
                <div className="flex items-center justify-between text-xs text-[var(--muted-foreground)] border-t border-[var(--border)] pt-3">
                  <div className="flex items-center gap-3">
                    <span className="flex items-center gap-1"><LayoutGrid className="w-3 h-3" /> {biz._count.products} products</span>
                    <span className="flex items-center gap-1"><Sparkles className="w-3 h-3" /> {biz._count.marketingAssets} assets</span>
                  </div>
                  <span>
